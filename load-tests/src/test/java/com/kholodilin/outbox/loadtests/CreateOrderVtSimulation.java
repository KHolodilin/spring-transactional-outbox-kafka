...
/**
 * Load test for {@code order-service-vt} ({@code POST /api/v1/orders} on port 8084).
 * <p>
 * Same knobs as {@link CreateOrderSimulation}; default {@code baseUrl} is {@code http://localhost:8084}.
 * Use {@code -Dprofile=warmup} for warmup → pause → steady load.
 */
public class CreateOrderVtSimulation extends Simulation {
    ...
    private final ScenarioBuilder scn = scenario("CreateOrderVt")
            .feed(feeder)
            .exec(
    ...
    public CreateOrderVtSimulation() {
